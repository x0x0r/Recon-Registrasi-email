...
def scan_login(url, user_field="username", pass_field="password", output_file="scan_results.txt"):
    ...
    for payload in test_payloads:
        ...
        if is_rfc822_valid("test@abcd.com"):  # pastikan domain valid
            ...
            save_valid_email(combined_email)  # <-- simpan email valid
    ...
    # simpan hasil valid ke file txt (payload+email)
    if valid_results:
...
if __name__ == "__main__":
    ...
    if is_rfc822_valid(email):
        ...
        save_valid_email(email)  # <-- simpan email valid dari input manual
    else:
